#### 13.4.1 Demo Storyboard

<div class="template-meta">
  <span class="stage-badge st-demo">Stage 04 · Demo</span>
  <a class="stage-badge" href="docs/13-templates/html/demo-storyboard.html" target="_blank" style="text-decoration:none">&#128424;&#65039; Printable HTML version</a>
</div>

A demo without a storyboard defaults to a harbor tour — every feature gets thirty seconds, and no feature gets remembered. This template operationalises [Abolishing the Harbor Tour](docs/05-demo-excellence/abolishing-harbor-tour.md), the [5-Minute Aha! Rule](docs/05-demo-excellence/aha-moment.md), [Day in the Life Narrative Construction](docs/05-demo-excellence/day-in-the-life-narrative.md), and the [30-Minute Pre-Flight Checklist](docs/05-demo-excellence/pre-flight-checklist.md).

| | |
|---|---|
| **When to use** | 2–3 days before any tailored demo; never for a demo you haven't done discovery for |
| **Owner** | SE |
| **Time to complete** | ~30 minutes |
| **Output goes to** | Yourself + AE; scene table optionally shared as the meeting agenda |

---

## The Template

```markdown
# Demo Storyboard — [Account] / [Date]

## 1. Frame
- **Audience & personas in the room:** [names, roles, what each cares about]
- **The ONE thing they must remember:** ______
- **Aha moment (must land before minute 5):** ______
- **Format:** [live / guided sandbox / video] — why: ______

## 2. Cold Open (minutes 0–3 — no agenda slide, no company history)
> "When we spoke on [date], [champion] told us [verbatim pain quote].
> Today we're going to show you that problem disappearing. Here's the before."

## 3. Scene Table
| # | Min | Scene (their workflow, not our module) | Pain it kills (from discovery) | Persona hero | Tension → Relief beat | Proof point shown |
|---|-----|----------------------------------------|--------------------------------|--------------|-----------------------|-------------------|
| 1 | 3–8 | | Pain #1 | | "Today this takes 4 hrs..." → "...now watch" | |
| 2 | 8–15 | | Pain #2 | | | |
| 3 | 15–22 | | Pain #3 | | | |
| — | 22–30 | Objections, next step close | | | | |

## 4. Deliberately NOT Showing
| Capability | Why cut |
|------------|---------|
| | Doesn't map to a discovered pain |

## 5. Anticipated Objections (top 3)
| Objection | Who'll raise it | Response (hold-and-validate, then...) |
|-----------|-----------------|----------------------------------------|
| | | |

## 6. Pre-Flight (T-30 minutes)
- [ ] Golden-state environment restored; demo data matches THEIR industry & terminology
- [ ] Notifications, Slack, bookmarks bar, extra tabs — hidden
- [ ] Crash recovery path rehearsed: if [scene 2] fails, pivot to ______
- [ ] AE aligned: who opens, who closes, who parks pricing
- [ ] Next-step ask scripted verbatim: ______

## 7. Post-Demo Capture (within 1 hour)
Reactions by persona: ______ · Objections raised: ______ · Agreed next step: ______
Self-score (see 5.6.1): Narrative /5 · Aha timing /5 · Objection handling /5
```

---

## How to Fill It In

1. **Scenes, not features.** Each row is a moment in *their* workday — "Priya's Monday-morning pipeline triage" — not a module name. If you can't name the persona who lives in the scene, cut the scene.
2. **The aha is a design constraint, not a hope.** Decide what it is, then ruthlessly clear everything in front of it. If your aha lands at minute 20, the executives have already mentally left the room.
3. **Section 4 is where discipline lives.** Every capability you cut is a capability that can't dilute the message. The strongest demos are defined by what they refuse to show.
4. **Rehearse the crash, not just the demo.** You don't get to choose whether the environment fails; you only choose whether you've [rehearsed the recovery](docs/05-demo-excellence/crash-recovery.md).

> **Failure Mode:** The storyboard that dies on contact with the first question. A stakeholder interrupts in scene 1, the SE follows them down a rabbit hole, and scenes 2–3 never happen. The storyboard includes your return path: answer, bridge — "and that connects to exactly what I'm about to show" — and get back on the arc. You own the narrative; questions are guests in it.

🟡 **Mid-Level** — Thirty minutes of storyboarding is the difference between a demo they watched and a story they retell to the people who weren't in the room.
